...
class Pheno4dVoxelizationtestDataset(VoxelizationDataset):
  ...
  DATA_PATH_FILE = {
      DatasetPhase.Train: 'Pheno4D_train.txt',
      DatasetPhase.Val: 'Pheno4D_val.txt',
      DatasetPhase.TrainVal: 'Pheno4D_trainval.txt',
      DatasetPhase.Test: 'Pheno4D_test.txt'
  }
  ...
  def __init__(self,
    ...
    data_root = config.pheno4d_test_path
    if phase not in [DatasetPhase.Train, DatasetPhase.TrainVal]:
      self.CLIP_BOUND = self.TEST_CLIP_BOUND
    data_paths = read_txt(os.path.join('./splits/Pheno4D', self.DATA_PATH_FILE[phase]))
...
class Pheno4dVoxelization2cmtestDataset(Pheno4dVoxelizationtestDataset):
  VOXEL_SIZE = 0.2